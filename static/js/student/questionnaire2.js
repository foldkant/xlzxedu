$(document).ready(function() {
	layui.use('form', function() {
		...
		function getQuestion() {
			$.ajax({
				...
				success: function(data) {
					...
					$('#question').append('<div class="layui-form-item"><div class="layui-input-block"><button class="layui-btn" lay-submit lay-filter="submit">确认提交</button><button type="reset" class="layui-btn layui-btn-primary">重置</button></div></div>');
					form.render();
				}
			})
		}
		form.on('submit(submit)', function(data) {
			...
			$.ajax({
				url: '../server/student/answer.php?lessonId=-2',
				dataType: 'json',
				type: 'post',
				data:{
					data:data.field
