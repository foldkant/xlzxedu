	...
	if($conn){
		...
		$countSql = "SELECT COUNT(*) FROM `course` WHERE `teacher_id`='{$teacherId}' ORDER BY `id` DESC";
		...
		$sql = "SELECT * FROM `course` WHERE `teacher_id`='{$teacherId}' ORDER BY `id` DESC LIMIT $startpos,$pageNum";
		 // echo($sql);
